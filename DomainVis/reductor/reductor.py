...
class BasePresenter():
	...
	def get_database(self, net, test_loaders, layer_name, threshold, img_size, mask_cut, upsampling, plot_mode):
		...
		np.save('data/' + str(layer_name) + 'eval_map', self.eval_maps)
		np.save('data/' + str(layer_name) + 'input', self.input)
...
class Reductor(BasePresenter):
		...
		for algo in algos:
			for mode in modes:
				...
				if algo == 'isomap':
					...
					if save:
						fig.write_image(
							"data/graphs/isomap/" + str(layer_name) + "_mode_" + str(isomap.mode) + "_thresh_" + str(
								threshold) + "_cut_" + str(mask_cut) + str(img_size) + ".jpg")
						np.save(
							"data/graphs/isomap/" + str(layer_name) + "_mode_" + str(isomap.mode) + "_thresh_" + str(
								threshold) + "_cut_" + str(mask_cut) + str(img_size), np.array(all_outputs))
						...
						if mode == 'pixel':
							...
							for dom in range(0, orig_shape[0]):
								outputs_subset = np.random.choice(range(len(outputs[dom])),
								                                  ...
								                                  replace=False)
								fig.add_trace(go.Scatter(x=outputs[dom, outputs_subset][:, 0],
								                         y=outputs[dom, outputs_subset][:, 1], name=self.domains[dom],
								                         mode='markers',
								                         opacity=0.7))
				...
				name = "data/graphs/" + str(algo)+ str(layer_name) + "_mode_" + str(pca.mode) + "_thresh_" + str(
					threshold) + "_cut_" + str(mask_cut) + str(img_size)+ '_eval_map'
				np.save(name, self.eval_maps)
	...
	def auto(algos, modes, layer_name, threshold, img_size, volume_num, sample_num, mask_cut,
	         perp, n_iter, save, path, names, full_model=True, data_path="", mask_path=""):
		...
		reductor.reduction(algos, modes, layer_name, threshold, img_size, volume_num, sample_num, mask_cut,
		                   perp, n_iter, save, data_path, mask_path)
...
if __name__ == '__main__':
	...
	# layers = ['init_path','up2', 'up3']
	layers = ['down1', 'down2', 'down3', 'up1', 'up2']
				...
				for img_size in img_sizes:
					for layer_name in layers:
						Reductor.auto(algos, modes, layer_name, threshold, img_size, volume_num, sample_num, mask_cut,
						              perp, n_iter, save, path, names, full_model=True, data_path=data_path,
						              mask_path=mask_path)
