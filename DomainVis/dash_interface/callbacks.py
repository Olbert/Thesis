...
def demo_callbacks(app):
            ...
            if ctx.triggered[0]['prop_id'].split('.')[0] == 'div-plot-click-image':
                ...
                if algo and page_on == 1:
                    ...
                    if current_id in cache:
                        ...
                        reductor = dim_reduction.Reductor.auto([algo], [mode], layer, None, (img_size, img_size),
                                                                      volumes, samples, mask_cut, perplexity,
                                                                      neighbours, n_iter,
        ...
        if page_on == 1:
            ...
            if ctx.triggered[0]['prop_id'].split('.')[0] != 'dropdown-mode' and mode == 'pixel':
                ...
                if len(points_list) == 0:
                    means = np.zeros(len(reductor['output']))
                    vars = np.zeros(len(reductor['output']))
                    for i in range(len(reductor['output'])):
                        means[i] = np.array(reductor['output'][i]).mean()
                        vars[i] = np.array(reductor['output'][i]).var()
                        # table_data =  pd.DataFrame(
                        table_data = {
                                "Domain": reductor['domains'],
                                "Mean": means,
                                "Variance": vars,
                            }
                        # )
                ...
                columns_ob = [{'id': 'from_mid', 'name': 'From Mid', 'type': 'numeric',},
                              {'id': 'price', 'name': 'Price'}, {'id': 'size', 'name': 'Size'},
                              {'id': 'cum_size', 'name': 'Size Total'},
                              {'id': 'size_$', 'name': 'Size $', 'type': 'numeric'},
                              {'id': 'cum_size_$', 'name': 'Size Total $', 'type': 'numeric'},
                              {'id': 'average_fill', 'name': 'Averge Fill'}, {'id': 'exc', 'name': 'Ex'},
                              {'id': 'side', 'name': 'side', 'hidden': True}],
                # order table data
                df = pd.DataFrame(np.random.randn(24, 9),
                                  columns=['from_mid', 'price', 'size', 'cum_size', 'size_$', 'cum_size_$',
                                           'average_fill', 'exc', 'side'])
    ...
    def upload_database(uploaded_filenames, uploaded_file_contents):
        ...
        if is_initial:
            ...
            return 'default', ['philips_15','philips_3','siemens_15', 'siemens_3','ge_15', 'ge_3', ]
        if uploaded_filenames is not None and uploaded_file_contents is not None:
            ...
            if (uploaded_filenames[0].split('.')[1] == 'h5'):
                ...
                full_dataset = H5Dataset.combine(full_names,
    ...
    def upload_label(uploaded_filenames, uploaded_file_contents):
        ...
        if uploaded_filenames is not None and uploaded_file_contents is not None:
            ...
            if (uploaded_filenames[0].split('.')[1] == 'h5'):
                ...
                full_dataset = H5Dataset.combine(full_paths,
                                                 os.path.join(config.UPLOAD_DIRECTORY, config.PROCESSED_DIRECTORY,
                                                              'label.h5'))
                names = H5Dataset.keys(full_dataset)
